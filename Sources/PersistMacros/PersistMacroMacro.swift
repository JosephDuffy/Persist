...
struct TransformerModifier: OptionSet {
    ...
    static let throwingInput = TransformerModifier(rawValue: 1 << 0)
    static let throwingOutput = TransformerModifier(rawValue: 1 << 1)
}
//
...
//        }
//
//        // TODO: Generate a `*Persiter` struct to contain the transformer and functions for get and set. Call these from the get/set of the actual property to prevent then need for a throwing setter. Will need to provide various persister types, e.g. throwing, throwing getter, throwing setter, no transformer, etc.
//
//        let labeledArguments = node.arguments?.as(LabeledExprListSyntax.self) ?? []
//
//        var storageExpression: ExprSyntax!
